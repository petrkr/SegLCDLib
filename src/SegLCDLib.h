...
/**
 * @mainpage SegLCDLib - Segment LCD Display Library
 *
 * Arduino library for segment LCD displays with support for multiple controllers.
 *
 * ## Features
 *
 * - **Multiple Controllers**: PCF85176 (I2C), HT1621/HT1622/VK0192 (3-wire serial)
 * - **Consistent Interface**: Follows Arduino LCD API 1.0 specification
 * - **Print Support**: Inherits from `Print` class for standard `print()`/`println()`
 * - **Display Variants**: Temperature/humidity, 4-6 digit, signal/battery indicators
 * - **RAM Efficient**: Buffered writes and minimal memory overhead
 *
 * ## Quick Start
 *
 * \code{.cpp}
 ...
 * - [Supported LCDs](../docs/supported-lcds.md) - LCD catalog with examples
 * - [Architecture](../docs/architecture.md) - Class hierarchy and design
 * - [Controllers](../docs/controllers.md) - Protocol specifications
 ...
 * |---------|-----------|----------|
 * | 6-Digit Temp/Humidity | PCF85176 | Icons, 6 digits |
 * | 6-Digit Signal/Battery | PCF85176 | Signal bars, battery |
 * | 4-Digit with Degree | HT1621 | Degree symbol, colon |
 * | 6-Digit with Battery | HT1621 | Battery indicator |
 * | 10-Digit 16-Segment | HT1622 | Enhanced segments |
 * | 5-Digit Signal/Battery | VK0192 | Advanced control |
 *
 * ## API Overview
 *
 * ### Display Control
 ...
 *
 * ### Character Output
 * - setCursor(row, col) - Position cursor
 * - print(value) - Output digit/string
 * - write(byte) - Output character
 *
 * ### Display Features
 * - setBattery(level) - Set battery indicator (0-3)
 * - setSignal(bars) - Set signal strength (0-4)
 * - Display-specific methods
 *
 * ## Architecture
 *
 * SegLCDLib uses a layered design:
 *
 * - **Base Class** (SegLCDLib): Abstract LCD interface
 * - **Controllers** (PCF85176, HT1621, etc.): Protocol handlers
 * - **LCD Implementations**: Display-specific drivers
 * - **Hardware**: I2C or 3-wire serial communication
 *
 * Each LCD class inherits from its controller because controllers are
 * integrated into LCD modules. This design minimizes memory and simplifies
 * the user API.
 *
 ...
 * ## Examples
 *
 * 13 complete examples included:
 * - PCF85176: 6 examples (TempHum, 6DigSigBat, OneDigit, T1T2, etc.)
 ...
 *
 * Access examples via **File** → **Examples** → **SegLCDLib** in Arduino IDE.
 *
 * ## Resources
 *
